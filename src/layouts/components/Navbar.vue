<template>
  <div class="navbar-container d-flex content align-items-center">
    ...
    <div class="navbar-header d-xl-block" v-if="surveyId && workflows.some(({ id }) => id === surveyId)">
      <ul class="nav navbar-nav">
        <li class="nav-item">
          <div class="navbar-brand">
            <h2 class="brand-text mb-0">{{ workflows.find(({ id }) => id === surveyId).name }}</h2>
          </div>
        </li>
      </ul>
    </div>
...
export default {
  ...
  computed: {
    ...mapState({ workflows: (state) => state.workflow.workflows }),
    surveyId() {
