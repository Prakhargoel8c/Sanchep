    ...
    <b-row class="auth-inner m-0">
      ...
      <b-col lg="4" class="d-flex align-items-center auth-bg px-2 p-lg-5">
        <b-col sm="8" md="6" lg="12" class="px-xl-2 mx-auto">
          <b-card-title title-tag="h2" class="font-weight-bold mb-1"> Set Password 🔒 </b-card-title>
          <b-card-text class="mb-2">
            Your new password must must contain at least one uppercase, one lowercase, one special character and one digit
          </b-card-text>
          ...
          <validation-observer ref="simpleRules">
            <b-form class="auth-reset-password-form mt-2" method="POST" @submit.prevent="validationForm">
              ...
              <!-- submit button -->
              <b-button block type="submit" variant="primary"> Set Password </b-button>
            </b-form>
          </validation-observer>
  ...
  methods: {
    ...
    validationForm() {
      this.$refs.simpleRules.validate().then((success) => {
        if (success) {
          useJwt
            .verifyAccount(this.token, this.password)
            .then(() => {
