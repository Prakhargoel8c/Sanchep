...
export default class JwtService {
  ...
  constructor(axiosIns, jwtOverrideConfig) {
    ...
    this.axiosIns.interceptors.request.use(
      (config) => {
        ...
        config.baseURL = 'http://chat.talkingmart.com:5004/api/';
        return config;
      },
  ...
  async submitSurvey(surveyData, surveyId) {
    const { data } = await this.axiosIns.get(this.jwtConfig.generateExecutedWorkflow.format(surveyId));
    await this.axiosIns.post(this.jwtConfig.submitExecutedWorkFlow.format(surveyId, data.workflow_url), surveyData);
    return data.id;
  }
  async getUwesSurveyData() {
    return (await this.axiosIns.get(this.jwtConfig.getUwesSurveyData)).data.filter((res) => res.workflow_id === '3356f22db50e472291d87eefe5260e7b');
  }
  async fogotPassword(email) {
    return this.axiosIns.post(this.jwtConfig.forgotPassword.format(email));
  }
  async resetPassword(token, password) {
    return this.axiosIns.post(this.jwtConfig.resetPassword, { token, new_password: password });
  }
